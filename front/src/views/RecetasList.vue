  ...
  <div class="min-h-screen bg-lime-50 flex flex-col">
    ...
    <section class="relative overflow-hidden">
      <div class="bg-gradient-to-br from-lime-100 via-lime-200 to-green-200 py-16 relative">
        ...
        <!-- Decoración con círculos animados -->
        <div class="absolute inset-0 overflow-hidden">
          <div
            class="absolute -left-10 -top-10 w-40 h-40 bg-yellow-300 rounded-full mix-blend-multiply filter blur-xl opacity-70 motion-safe:animate-[blob_7s_infinite]">
          </div>
          <div
            class="absolute -right-10 -top-10 w-40 h-40 bg-lime-300 rounded-full mix-blend-multiply filter blur-xl opacity-70 motion-safe:animate-[blob_7s_infinite_2s]">
          </div>
          <div
            class="absolute -bottom-10 left-20 w-40 h-40 bg-green-300 rounded-full mix-blend-multiply filter blur-xl opacity-70 motion-safe:animate-[blob_7s_infinite_4s]">
          </div>
        </div>
    ...
    <div class="w-full px-6 -mt-12 sm:-mt-8 md:-mt-6 relative z-20 flex justify-center items-center gap-4">
      <div class="w-full sm:w-2/3 md:w-1/2 lg:w-1/3 transform hover:scale-105 transition-transform duration-300">
        <div class="relative">
          <input type="text" v-model="searchTerm" placeholder="Cerca receptes..."
    ...
    <div class="max-w-7xl mx-auto px-6 py-6 mb-16">
      ...
      <!-- Estado de error -->
      <div v-else-if="error"
        class="bg-red-50 rounded-xl p-8 text-center border border-red-200 shadow-lg hover:shadow-xl transition-all duration-300 motion-safe:animate-[shake_0.5s_ease-in-out]">
        <div class="text-4xl mb-4">😕</div>
        <p class="text-red-600 mb-4 font-medium">{{ error }}</p>
        <button @click="fetchRecetas"
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="receta in filteredRecetas" :key="receta.id"
          ...
          <div class="p-6">
            ...
            <div class="flex flex-wrap gap-2 mb-3">
              ...
              <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800">{{ receta.cuisine?.country
                }}</span>
            </div>
            ...
            <div class="flex justify-between items-center">
              ...
              <button @click="showDeleteModal(receta)"
                class="px-3 py-1.5 text-xs rounded font-semibold bg-red-500 text-white hover:bg-red-600 transition">
                Eliminar
              </button>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      showModal: false,     // Mostrar modal de confirmación
      recetaToDelete: null, // Receta seleccionada para eliminar
      successMessage: null, // Mensaje de éxito
      showSuccessModal: false, // Mostrar modal de éxito
  ...
  methods: {
    ...
    /**
     * Muestra el modal de confirmación para eliminar una receta
    ...
    /**
     * Cancela la eliminación de la receta
     */
    cancelDelete() {
    ...
    /**
     * Confirma y ejecuta la eliminación de la receta
     */
    async confirmDelete() {
      try {
    ...
    /**
     * Acorta la descripción si es demasiado larga
     * @param {string} desc - Descripción a truncar
